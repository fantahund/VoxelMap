   ...
   public void populateDimensions(World world) {
      ...
      for(RegistryKey vanillaWorldKey : this.vanillaWorlds) {
         ...
         DimensionContainer dimensionContainer = new DimensionContainer(
            dimensionType, vanillaWorldKey.getValue().getPath(), vanillaWorldKey.getValue()
         );
         ...
         this.dimensions,
              (dim1, dim2) -> {
                 if (dim1.resourceLocation.equals(World.OVERWORLD.getValue())) {
                    return -1;
                 } else if (dim1.resourceLocation.equals(World.NETHER.getValue())
                    && !dim2.resourceLocation.equals(World.OVERWORLD.getValue())) {
                    return -1;
                 } else {
                    return dim1.resourceLocation.equals(World.END.getValue())
                          && !dim2.resourceLocation.equals(World.OVERWORLD.getValue())
                          && !dim2.resourceLocation.equals(World.NETHER.getValue())
                       ? -1
                       : collator.compare(dim1.name, dim2.name);
                 }
              }
      );
   }
