...
public class CommandUtils {
      ...
      if (waypointStrings.size() <= 0) {
         ...
         for(String waypointString : waypointStrings) {
            ...
            chatStyle = chatStyle.withClickEvent(
               new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/newWaypoint " + waypointString.substring(1, waypointString.length() - 1))
            );
            chatStyle = chatStyle.withColor(Formatting.AQUA);
            LiteralText hover = new LiteralText(
   ...
   public static void waypointClicked(String command) {
      boolean control = InputUtil.isKeyPressed(
            MinecraftClient.getInstance().getWindow().getHandle(), InputUtil.fromTranslationKey("key.keyboard.left.control").getCode()
         )
         || InputUtil.isKeyPressed(MinecraftClient.getInstance().getWindow().getHandle(), InputUtil.fromTranslationKey("key.keyboard.right.control").getCode());
      ...
      for(Waypoint wp : AbstractVoxelMap.getInstance().getWaypointManager().getWaypoints()) {
         if (wp.name.equalsIgnoreCase(details) && wp.inDimension && wp.inWorld) {
            ...
            MinecraftClient.getInstance()
               .player
               .sendChatMessage("/tp " + MinecraftClient.getInstance().player.getName().getString() + " " + wp.getX() + " " + y + " " + wp.getZ());
            if (mp) {
