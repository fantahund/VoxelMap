...
public class WorldUpdateListener {
    ...
    public void addListener(IChangeObserver chunkProcessor) { chunkProcessors.add(chunkProcessor); }

    public void notifyObservers(int chunkX, int chunkZ) {
        try {
            for (IChangeObserver chunkProcessor : this.chunkProcessors) chunkProcessor.handleChangeInWorld(chunkX, chunkZ);
