...
public class Map implements Runnable, IMap {
   ...
   public void onTickInGame(MatrixStack matrixStack, MinecraftClient mc) {
      ...
      if (this.game.currentScreen == null && this.options.keyBindWaypoint.wasPressed()) {
         ...
         Waypoint newWaypoint = new Waypoint(
            "",
            (int)((double)GameVariableAccessShim.xCoord() * dimensionScale),
            (int)((double)GameVariableAccessShim.zCoord() * dimensionScale),
   ...
   public void calculateCurrentLightAndSkyColor() {
      if (this.world != null) {
         ...
         int biomeID = this.world
            ...
            .getRawId(
               this.world
                  .getBiome(this.blockPos.withXYZ(GameVariableAccessShim.xCoord(), GameVariableAccessShim.yCoord(), GameVariableAccessShim.zCoord()))
            );
   ...
   private int getSkyColor() {
      ...
      if (!aboveHorizon && this.game.options.viewDistance >= 4) {
         ...
         if (sunsetColors != null && this.game.options.viewDistance >= 4) {
            int sunsetColor = (int)(sunsetColors[3] * 128.0F) * 16777216
               + (int)(sunsetColors[0] * 255.0F) * 65536
               + (int)(sunsetColors[1] * 255.0F) * 256
               + (int)(sunsetColors[2] * 255.0F);
   ...
   private void mapCalc(boolean full) {
      ...
      if (this.game.player.world.getDimension().hasCeiling()) {
         ...
         netherPlayerInOpen = this.world
               .getChunk(this.blockPos)
               .sampleHeightmap(Heightmap.Type.MOTION_BLOCKING, this.blockPos.getX() & 15, this.blockPos.getZ() & 15)
            <= currentY;
         nether = currentY < 126;
   ...
   public void processChunk(WorldChunk chunk) {
      this.rectangleCalc(
         chunk.getPos().x * 16,
         chunk.getPos().z * 16,
         chunk.getPos().x * 16 + 15,
         chunk.getPos().z * 16 + 15
      );
   }
   ...
   private int getPixelColor(
      ...
      if (needBiome) {
         if (world.isChunkLoaded(this.blockPos)) {
            biomeID = world.getRegistryManager().get(Registry.BIOME_KEY).getRawId(world.getBiome(this.blockPos));
         } else {
            biomeID = -1;
         }
         ...
         if (needHeightAndID) {
            if (!nether && !caves) {
               ...
               if (material == Material.WATER || material == Material.ICE) {
                  ...
                  for(this.seafloorBlockState = world.getBlockState(this.blockPos.withXYZ(startX + imageX, surfaceHeight - 1, startZ + imageY));
               ...
               if (material != Material.SNOW_LAYER
                  && material != Material.AIR
   ...
   private void renderMap(MatrixStack matrixStack, int x, int y, int scScale) {
      ...
      GLShim.glScissor(
         (int)(guiScale * (double)(x - 32)), (int)(guiScale * ((double)(this.scHeight - y) - 32.0)), (int)(guiScale * 64.0), (int)(guiScale * 63.0)
      );
      GLUtils.drawPre();
      ...
      for(Waypoint pt : this.waypointManager.getWaypoints()) {
         if (pt.isActive() || pt == highlightedPoint) {
            ...
            if (distanceSq < (double)(this.options.maxWaypointDisplayDistance * this.options.maxWaypointDisplayDistance)
   ...
   private void drawArrow(MatrixStack matrixStack, int x, int y) {
      try {
         ...
         matrixStack.multiply(
            Vec3f.POSITIVE_Z.getDegreesQuaternion(this.options.rotates && !this.fullscreenMap ? 0.0F : this.direction + (float)this.northRotate)
         );
         matrixStack.translate((double)(-x), (double)(-y), 0.0);
         RenderSystem.applyModelViewMatrix();
         GLUtils.drawPre();
   ...
   private void drawDirections(MatrixStack matrixStack, int x, int y) {
      ...
      matrixStack.translate(
         (double)distance * Math.sin(Math.toRadians(-((double)rotate - 90.0))), (double)distance * Math.cos(Math.toRadians(-((double)rotate - 90.0))), 100.0
      );
      ...
      matrixStack.scale(scale, scale, 1.0F);
      matrixStack.translate(
         (double)distance * Math.sin(Math.toRadians((double)(-rotate))), (double)distance * Math.cos(Math.toRadians((double)(-rotate))), 10.0
      );
      ...
      matrixStack.scale(scale, scale, 1.0F);
      matrixStack.translate(
         (double)distance * Math.sin(Math.toRadians(-((double)rotate + 90.0))), (double)distance * Math.cos(Math.toRadians(-((double)rotate + 90.0))), 10.0
      );
      ...
      matrixStack.scale(scale, scale, 1.0F);
      matrixStack.translate(
         (double)distance * Math.sin(Math.toRadians(-((double)rotate + 180.0))), (double)distance * Math.cos(Math.toRadians(-((double)rotate + 180.0))), 10.0
      );
   ...
   private void drawWelcomeScreen(MatrixStack matrixStack, int scWidth, int scHeight) {
      if (this.welcomeText[1] == null || this.welcomeText[1].getString().equals("minimap.ui.welcome2")) {
         this.welcomeText[0] = (new LiteralText(""))
            .append((new LiteralText("VoxelMap! ")).formatted(Formatting.RED))
            .append(this.zmodver + " ")
            .append(new TranslatableText("minimap.ui.welcome1"));
         ...
         this.welcomeText[4] = (new LiteralText(""))
            .append((new KeybindText(this.options.keyBindZoom.getTranslationKey())).formatted(Formatting.AQUA))
            .append(": ")
            .append(new TranslatableText("minimap.ui.welcome5a"))
            .append(", ")
            .append((new KeybindText(this.options.keyBindMenu.getTranslationKey())).formatted(Formatting.AQUA))
            .append(": ")
            .append(new TranslatableText("minimap.ui.welcome5b"));
         this.welcomeText[5] = (new LiteralText(""))
            .append((new KeybindText(this.options.keyBindFullscreen.getTranslationKey())).formatted(Formatting.AQUA))
            .append(": ")
            .append(new TranslatableText("minimap.ui.welcome6"));
         this.welcomeText[6] = (new LiteralText(""))
            .append((new KeybindText(this.options.keyBindWaypoint.getTranslationKey())).formatted(Formatting.AQUA))
            .append(": ")
            .append(new TranslatableText("minimap.ui.welcome7"));
         this.welcomeText[7] = this.options
            .keyBindZoom
            .getBoundKeyLocalizedText()
            .shallowCopy()
            .append(": ")
            .append((new TranslatableText("minimap.ui.welcome8")).formatted(Formatting.GRAY));
      }
