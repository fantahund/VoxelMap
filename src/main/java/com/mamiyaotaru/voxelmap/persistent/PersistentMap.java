...
public class PersistentMap implements IPersistentMap, IChangeObserver {
   ...
   Comparator distanceSorter = (Comparator<RegionCoordinates>) (coordinates1, coordinates2) -> {
      double distance1sq = (double)(
         (coordinates1.x * 256 + 128 - PersistentMap.this.options.mapX) * (coordinates1.x * 256 + 128 - PersistentMap.this.options.mapX)
            + (coordinates1.z * 256 + 128 - PersistentMap.this.options.mapZ) * (coordinates1.z * 256 + 128 - PersistentMap.this.options.mapZ)
      );
      double distance2sq = (double)(
         (coordinates2.x * 256 + 128 - PersistentMap.this.options.mapX) * (coordinates2.x * 256 + 128 - PersistentMap.this.options.mapX)
      ...
      if (biomeID != -1) {
         ...
         if (underground) {
            ...
            if (material == Material.WATER || material == Material.ICE) {
               ...
               for(seafloorBlockState = chunk.getBlockState(blockPos.withXYZ(startX + imageX, surfaceHeight - 1, startZ + imageY));
                  seafloorBlockState.getOpacity(world, blockPos) < 5 && seafloorBlockState.getMaterial() != Material.LEAVES && seafloorHeight > 1;
                  seafloorBlockState = chunk.getBlockState(blockPos.withXYZ(startX + imageX, seafloorHeight - 1, startZ + imageY))
               ) {
