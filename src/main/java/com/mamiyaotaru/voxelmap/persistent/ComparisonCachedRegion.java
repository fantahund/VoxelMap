   ...
   public void loadCurrent() {
      ...
      for(int chunkX = 0; chunkX < 16; ++chunkX) {
         for(int chunkZ = 0; chunkZ < 16; ++chunkZ) {
            ...
            if (chunk != null
               && !chunk.isEmpty()
               && this.world.isChunkLoaded(this.x * 16 + chunkX, this.z * 16 + chunkZ)
               && !this.isChunkEmpty(this.world, chunk)) {
         ...
         if (cachedRegionFile.exists()) {
            ...
            for(decompressedByteData = new byte[this.data.getWidth() * this.data.getHeight() * 17 * 4]; (ze = zis.getNextEntry()) != null; zis.closeEntry()) {
               ...
               if (ze.getName().equals("data")) {
                  for(byte[] data = new byte[2048];
                     (count = zis.read(data, 0, 2048)) != -1 && count + total <= this.data.getWidth() * this.data.getHeight() * 17 * 4;
                     total += count
                  ) {
      ...
      for(int t = 0; t < 16; ++t) {
         for(int s = 0; s < 16; ++s) {
            ...
            MessageUtils.printDebug(
               "at " + t + "," + s + " there were local non zero: " + nonZeroHeights + " and comparison non zero: " + nonZeroHeightsInCandidate
            );
         }
      }
