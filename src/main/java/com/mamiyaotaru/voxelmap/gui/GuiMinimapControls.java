   ...
   public void init() {
      ...
      for(int t = 0; t < this.options.keyBindings.length; ++t) {
         ...
         this.addDrawableChild(
            new ButtonWidget(
               left + t % 2 * 160,
               this.getHeight() / 6 + 24 * (t >> 1),
               70,
               20,
               this.options.getKeybindDisplayString(t),
               button -> this.controlButtonClicked(id)
            )
         );
      }
   ...
   public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
      ...
      for(int keyCounter = 0; keyCounter < this.options.keyBindings.length; ++keyCounter) {
         ...
         if (this.buttonId == this.options.keyBindings[keyCounter]) {
            ((ButtonWidget)this.getButtonList().get(keyCounter))
               .setMessage(
                  (new LiteralText("> "))
                     .append((new LiteralText("???")).shallowCopy().formatted(Formatting.YELLOW))
                     .append(" <")
                     .formatted(Formatting.YELLOW)
               );
         } else if (keycodeCollision) {
            ((ButtonWidget)this.getButtonList().get(keyCounter))
               .setMessage(this.options.getKeybindDisplayString(keyCounter).shallowCopy().formatted(Formatting.RED));
         } else {
      ...
      drawCenteredText(
         matrixStack, this.getFontRenderer(), I18nUtils.getString("controls.minimap.unbind1"), this.getWidth() / 2, this.getHeight() / 6 + 115, 16777215
      );
      drawCenteredText(
         matrixStack, this.getFontRenderer(), I18nUtils.getString("controls.minimap.unbind2"), this.getWidth() / 2, this.getHeight() / 6 + 129, 16777215
      );
