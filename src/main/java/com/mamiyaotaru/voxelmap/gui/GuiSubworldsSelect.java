...
public class GuiSubworldsSelect extends GuiScreenMinimap implements BooleanConsumer {
   ...
   public GuiSubworldsSelect(Screen parent, IVoxelMap master) {
      ...
      this.camera = new ClientPlayerEntity(
         this.getMinecraft(), this.getMinecraft().world, this.getMinecraft().getNetworkHandler(), this.thePlayer.getStatHandler(), new ClientRecipeBook(), false, false
      );
      this.camera.input = new KeyboardInput(this.getMinecraft().options);
      this.camera
         .refreshPositionAndAngles(
            this.thePlayer.getX(),
            this.thePlayer.getY() - this.thePlayer.getHeightOffset(),
            this.thePlayer.getZ(),
            this.thePlayer.getYaw(),
            0.0F
         );
   ...
   public void init() {
      ...
      this.cancelBtn = new ButtonWidget(
         centerX - 100, this.height - 30, 200, 20, new TranslatableText("gui.cancel"), button -> this.getMinecraft().setScreen((Screen)null)
      );
      ...
      int lastRowShiftBy = (int)(
         Math.ceil((double)((float)(numKnownSubworlds + 1) / (float)buttonsPerRow)) * (double)buttonsPerRow - (double)(numKnownSubworlds + 1)
      );
      ...
      for(int t = 0; t < numKnownSubworlds; ++t) {
         ...
         this.selectButtons[t] = new ButtonWidget(
            ...
            buttonWidth - 32,
            20,
            new LiteralText(this.worlds[t]),
            button -> this.worldSelected(this.worlds[tt])
         );
         this.editButtons[t] = new ButtonWidget(
            ...
            20,
            new LiteralText("⚒"),
            button -> this.editWorld(this.worlds[tt])
         );
      ...
      if (!this.newWorld) {
         this.selectButtons[numButtons] = new ButtonWidget(
            ...
            buttonWidth - 2,
            20,
            new LiteralText("< " + I18nUtils.getString("worldmap.multiworld.newname") + " >"),
            button -> {
               this.newWorld = true;
