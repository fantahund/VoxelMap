...
class GuiSlotWaypoints extends GuiSlotMinimap {
   ...
   public void setSelected(WaypointItem item) {
      ...
      if (this.getSelectedOrNull() instanceof WaypointItem) {
         NarratorManager.INSTANCE
            .narrate((new TranslatableText("narrator.select", new Object[]{((WaypointItem)this.getSelectedOrNull()).waypoint.name})).getString());
      }
   ...
   public void drawTexturedModalRect(int xCoord, int yCoord, Sprite textureSprite, int widthIn, int heightIn) {
      ...
      vertexbuffer.vertex((double)(xCoord + 0), (double)(yCoord + heightIn), 1.0)
         .texture(textureSprite.getMinU(), textureSprite.getMaxV())
         .next();
      vertexbuffer.vertex((double)(xCoord + widthIn), (double)(yCoord + heightIn), 1.0)
         .texture(textureSprite.getMaxU(), textureSprite.getMaxV())
         .next();
      vertexbuffer.vertex((double)(xCoord + widthIn), (double)(yCoord + 0), 1.0)
         .texture(textureSprite.getMaxU(), textureSprite.getMinV())
         .next();
   ...
   protected void sortBy(int sortKey, boolean ascending) {
      ...
      if (sortKey == 1) {
         ...
         Collections.sort(
            this.waypoints,
            new Comparator<WaypointItem>() {
               public int compare(WaypointItem waypointEntry1, WaypointItem waypointEntry2) {
                  return Double.compare(
                        (double)masterWaypointsList.indexOf(waypointEntry1.waypoint), (double)masterWaypointsList.indexOf(waypointEntry2.waypoint)
                     )
                     * order;
               }
            }
   ...
   public class WaypointItem extends EntryListWidget.Entry<WaypointItem> implements Comparable<WaypointItem> {
      ...
      public void render(
         ...
         DrawableHelper.drawCenteredText(
            matrixStack, this.parentGui.getFontRenderer(), this.waypoint.name, this.parentGui.getWidth() / 2, slotYPos + 3, this.waypoint.getUnifiedColor()
         );
         byte padding = 3;
