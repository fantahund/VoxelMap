...
public class GuiWaypoints extends GuiScreenMinimap implements IGuiWaypoints {
   ...
   public void init() {
      ...
      this.addDrawableChild(
         this.buttonSortName = new ButtonWidget(
            this.getWidth() / 2 - 154, 34, 77, 20, new TranslatableText("minimap.waypoints.sortbyname"), button -> this.sortClicked(2)
         )
      );
      this.addDrawableChild(
         this.buttonSortDistance = new ButtonWidget(
            this.getWidth() / 2 - 77, 34, 77, 20, new TranslatableText("minimap.waypoints.sortbydistance"), button -> this.sortClicked(3)
         )
      );
      this.addDrawableChild(
         this.buttonSortCreated = new ButtonWidget(
            this.getWidth() / 2, 34, 77, 20, new TranslatableText("minimap.waypoints.sortbycreated"), button -> this.sortClicked(1)
         )
      );
      this.addDrawableChild(
         this.buttonSortColor = new ButtonWidget(
            this.getWidth() / 2 + 77, 34, 77, 20, new TranslatableText("minimap.waypoints.sortbycolor"), button -> this.sortClicked(4)
         )
      );
      ...
      this.filter = new TextFieldWidget(
         this.getFontRenderer(), this.getWidth() / 2 - 153 + filterStringWidth + 5, this.getHeight() - 80, 305 - filterStringWidth - 5, 20, (Text)null
      );
      this.filter.setMaxLength(35);
      this.addDrawableChild(this.filter);
      this.addDrawableChild(
         this.buttonEdit = new ButtonWidget(
            this.getWidth() / 2 - 154, this.getHeight() - 52, 74, 20, new TranslatableText("selectServer.edit"), button -> this.editWaypoint(this.selectedWaypoint)
         )
      );
      this.addDrawableChild(
         this.buttonDelete = new ButtonWidget(
            this.getWidth() / 2 - 76, this.getHeight() - 52, 74, 20, new TranslatableText("selectServer.delete"), button -> this.deleteClicked()
         )
      );
      this.addDrawableChild(
         this.buttonHighlight = new ButtonWidget(
            this.getWidth() / 2 + 2, this.getHeight() - 52, 74, 20, new TranslatableText("minimap.waypoints.highlight"), button -> this.setHighlightedWaypoint()
         )
      );
      this.addDrawableChild(
         this.buttonTeleport = new ButtonWidget(
            this.getWidth() / 2 + 80, this.getHeight() - 52, 74, 20, new TranslatableText("minimap.waypoints.teleportto"), button -> this.teleportClicked()
         )
      );
      this.addDrawableChild(
         this.buttonShare = new ButtonWidget(
            this.getWidth() / 2 - 154,
            this.getHeight() - 28,
            74,
            20,
            new TranslatableText("minimap.waypoints.share"),
            button -> CommandUtils.sendWaypoint(this.selectedWaypoint)
         )
      );
      this.addDrawableChild(
         new ButtonWidget(this.getWidth() / 2 - 76, this.getHeight() - 28, 74, 20, new TranslatableText("minimap.waypoints.newwaypoint"), button -> this.addWaypoint())
      );
      this.addDrawableChild(
         new ButtonWidget(
            this.getWidth() / 2 + 2,
            ...
            button -> this.getMinecraft().setScreen(new GuiWaypointsOptions(this, this.options))
         )
      );
      this.addDrawableChild(
         new ButtonWidget(
            this.getWidth() / 2 + 80, this.getHeight() - 28, 74, 20, new TranslatableText("gui.done"), button -> this.getMinecraft().setScreen(this.parentScreen)
         )
      );
   ...
   private void teleportClicked() {
      ...
      int y = this.selectedWaypoint.getY() > 0
         ...
         : (!this.options.game.player.world.getDimension().hasCeiling() ? 255 : 64);
      this.options
         .game
         .player
         .sendChatMessage(
            "/tp "
               + this.options.game.player.getName().getString()
               + " "
   ...
   public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
      ...
      drawStringWithShadow(
         ...
         I18nUtils.getString("minimap.waypoints.filter") + ":",
         this.getWidth() / 2 - 153,
         this.getHeight() - 75,
         10526880
      );
